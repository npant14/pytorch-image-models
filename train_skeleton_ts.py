...
def train_one_epoch(
    ...
    model_time_m = utils.AverageMeter()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        model_time_m.update(time.time() - model_start_time)
        _backward(loss)
        ...
        if update_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                _logger.info(
                    ...
                    f'Model Time: {model_time_m.val:.3f}s, {update_sample_count / model_time_m.val:>7.2f}/s  '
                    f'({model_time_m.avg:.3f}s, {update_sample_count / model_time_m.avg:>7.2f}/s)  '
                    f'Update Time: {update_time_m.val:.3f}s, {update_sample_count / update_time_m.val:>7.2f}/s  '
                    f'({update_time_m.avg:.3f}s, {update_sample_count / update_time_m.avg:>7.2f}/s)  '
                    f'LR: {lr:.3e}  '
                    f'Data time: {data_time_m.val:.3f} ({data_time_m.avg:.3f} /s )'
                )
