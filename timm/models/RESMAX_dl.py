...
class RESMAX_V2_2_DL_1(nn.Module):
    def __init__(self, num_classes=1000, big_size=322, small_size=227, in_chans=3, 
                 ip_scale_bands=1, classifier_input_size=13312, contrastive_loss=False, pyramid=False,
                 bypass=False, main_route=False,
                 c_scoring='v2',
        ...
        self.c_scoring = c_scoring
        self.main_route = main_route
        super(RESMAX_V2_2_DL_1, self).__init__()
    ...
    def forward(self, x, main_route=False, pyramid=False):
...
"""
class CHRESMAX_V3_1_DL(nn.Module):
    """
    ...
    def __init__(self, 
        ...
        self.model_backbone = RESMAX_V2_2_DL_1(
    ...
    def make_reference_scale(self, x, scale_factor_list, scale_band):
        base_image_size = int(x.shape[-1])
        image_scales = scale_factor_list
        x_reference = torch.zeros_like(x)
        for i in range(len(scale_band)):
            x_i = x[i:i+1]  # Keep batch dimension
            img_hw = x.shape[-1]
            resize_size = int(img_hw * image_scales[scale_band[i]])
            x_i = F.interpolate(x_i, size=(resize_size, resize_size), mode='bilinear', align_corners=False)
            # pad if it less than base_image_size or crop if it greater than base_image_size
            if x_i.shape[-1] < base_image_size:
                x_i = pad_to_size(x_i, (base_image_size, base_image_size))
    ...
    def forward(self, x, scale_band=None):
        ...
        # stream 2  using scale_band
        scale_factor_list = [0.49, 0.59, 0.707, 0.841, 1.0, 1.189, 1.414, 1.681, 2.0]

        x_rescaled = self.make_reference_scale(x, scale_factor_list, scale_band)
...
class CHRESMAX_V3_3_DL(nn.Module):
    """
    Example student-teacher style model with scale-consistency loss,
    using RESMAX_V2 as the backbone.
    ...
    def forward(self, x, scale_band=None):
        ...
        c1_correct_scale_loss = 0
        for i in range(len(stream_1_c1_feats)):
            # Flatten the features for cosine similarity
            f1 = stream_1_c1_feats[i].view(stream_1_c1_feats[i].size(0), -1)
            f2 = stream_2_c1_feats[i].view(stream_2_c1_feats[i].size(0), -1)
            # Normalize the features
            f1 = F.normalize(f1, p=2, dim=1)
            f2 = F.normalize(f2, p=2, dim=1)
            # Compute cosine similarity (1 - similarity since we want to minimize the loss)
            c1_correct_scale_loss += (1 - torch.sum(f1 * f2, dim=1)).mean()
        c1_correct_scale_loss /= len(stream_1_c1_feats)  # Average over all feature maps
        
        c2_correct_scale_loss = 0
        for i in range(len(stream_1_c2_feats)):
            # Flatten the features for cosine similarity
            f1 = stream_1_c2_feats[i].view(stream_1_c2_feats[i].size(0), -1)
            f2 = stream_2_c2_feats[i].view(stream_2_c2_feats[i].size(0), -1)
            # Normalize the features
            f1 = F.normalize(f1, p=2, dim=1)
            f2 = F.normalize(f2, p=2, dim=1)
            # Compute cosine similarity (1 - similarity since we want to minimize the loss)
            c2_correct_scale_loss += (1 - torch.sum(f1 * f2, dim=1)).mean()
        c2_correct_scale_loss /= len(stream_1_c2_feats)  # Average over all feature maps
        
        # For output features
        f1 = stream_1_output.view(stream_1_output.size(0), -1)
        f2 = stream_2_output.view(stream_2_output.size(0), -1)
        f1 = F.normalize(f1, p=2, dim=1)
        f2 = F.normalize(f2, p=2, dim=1)
        out_correct_scale_loss = (1 - torch.sum(f1 * f2, dim=1)).mean()

        if self.bypass:
            # For bypass features
            f1 = stream_1_bypass.view(stream_1_bypass.size(0), -1)
            f2 = stream_2_bypass.view(stream_2_bypass.size(0), -1)
            f1 = F.normalize(f1, p=2, dim=1)
            f2 = F.normalize(f2, p=2, dim=1)
            bypass_correct_scale_loss = (1 - torch.sum(f1 * f2, dim=1)).mean()
        else:
            bypass_correct_scale_loss = 0
...
@register_model
def chresmax_v3_3_dl(pretrained=False, **kwargs):
    try:
        del kwargs["pretrained_cfg"]
        del kwargs["pretrained_cfg_overlay"]
        del kwargs["drop_rate"]
        ...
        pass
    model = CHRESMAX_V3_3_DL(**kwargs)
    if pretrained:
        pass
    return model

@register_model
def chresmax_v3_1_dl(pretrained=False, **kwargs):
    try:
        del kwargs["pretrained_cfg"]
        del kwargs["pretrained_cfg_overlay"]
        del kwargs["drop_rate"]
